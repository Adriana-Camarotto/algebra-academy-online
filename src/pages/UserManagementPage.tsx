  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      const emailSent = await sendWelcomeEmail({
        ...
        from_name: "Algebra Academy Team",
      });
      ...
      if (emailSent) {
        toast({
          ...
          description:
            language === "en"
              ? `User ${formData.name} created successfully! Welcome email sent to ${formData.email}`
              : `Usuário ${formData.name} criado com sucesso! Email de boas-vindas enviado para ${formData.email}`,
          duration: 8000,
        });
      } else {
        toast({
          title: language === "en" ? "Partial Success" : "Sucesso Parcial",
          description:
            language === "en"
              ? `User ${formData.name} created successfully! EmailJS not configured - instructions copied to clipboard. Configure EmailJS in 3 minutes: see CONFIGURE_EMAILJS_NOW.md`
              : `Usuário ${formData.name} criado com sucesso! EmailJS não configurado - instruções copiadas. Configure EmailJS em 3 minutos: veja CONFIGURE_EMAILJS_NOW.md`,
          variant: "destructive",
          duration: 12000,
        });

        // Fallback: Copy instructions to clipboard
        const instructions =
          language === "en"
            ? `Hello ${formData.name},
...
Bem-vindo à Algebra Academy!`;
        ...
        try {
          ...
          toast({
            ...
            description:
              language === "en"
                ? "Registration instructions copied to clipboard as backup."
                : "Instruções de registro copiadas como backup.",
            duration: 5000,
          });
        } catch (e) {
          console.log("Could not copy to clipboard:", e);
              ...
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-sm text-gray-600">
                    {language === "en"
                      ? "Create a user profile. A welcome email with registration instructions will be automatically sent to the user's email address."
                      : "Crie um perfil de usuário. Um email de boas-vindas com instruções de registro será automaticamente enviado para o endereço de email do usuário."}
                  </DialogDescription>
                </DialogHeader>
