...
const StudentProgressPage = () => {
  ...
  const skillsData = [
    ...
    {
      subject: language === 'en' ? 'Mental Math' : 'Cálculo Mental',
      score: 90,
      fullMark: 100,
    },
    {
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">
          {language === 'en' ? 'My Learning Progress' : 'Meu Progresso de Aprendizado'}
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {language === 'en' ? 'Progress Over Time' : 'Progresso ao Longo do Tempo'}
            </CardTitle>
            <CardDescription>
              {language === 'en'
            ...
            <ChartContainer className="h-[350px]" config={{}}>
              <LineChart data={progressOverTime}>
                ...
                <Line 
                  ...
                  strokeWidth={2} 
                  activeDot={{ r: 8 }} 
                  name={language === 'en' ? 'Performance Score' : 'Pontuação de Desempenho'}
                />
              </LineChart>
            </ChartContainer>
          </CardContent>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>
              {language === 'en' ? 'Skills Assessment' : 'Avaliação de Habilidades'}
            </CardTitle>
            <CardDescription>
              {language === 'en'
          ...
          <CardContent>
            <ChartContainer className="h-[400px]" config={{}}>
              <RadarChart outerRadius={150} width={500} height={400} data={skillsData}>
                <PolarGrid />
                <PolarAngleAxis dataKey="subject" />
                <PolarRadiusAxis domain={[0, 100]} />
                <Radar 
                  name={language === 'en' ? 'Skills Level' : 'Nível de Habilidades'} 
                  dataKey="score" 
                  stroke="#8884d8" 
                  fill="#8884d8" 
                  fillOpacity={0.6} 
