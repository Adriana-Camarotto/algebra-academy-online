...
const SignupPage: React.FC = () => {
  const { signUpWithSupabase, language } = useAuthStore();
  ...
  const validateForm = () => {
    const schema = z
      .object({
        name: z.string().min(2, {
          message:
            language === "en" ? "Name is required" : "Nome é obrigatório",
        }),
        email: z.string().email({
          message:
            language === "en" ? "Invalid email address" : "Email inválido",
        }),
        ...
        <Card className="border-tutor-accent/20">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-center">
              {selectedRole
                ? `${language === "en" ? "Sign up as" : "Registrar como"} ${t(
                    selectedRole,
                    language
                  )}`
          ...
          <CardFooter className="flex flex-col gap-4">
            ...
            <p className="text-center text-sm text-muted-foreground">
              {language === "en"
                ...
                : "Já tem uma conta? "}
              <a href="/login" className="text-tutor-primary hover:underline">
                {t("login", language)}
              </a>
            </p>
          </CardFooter>
