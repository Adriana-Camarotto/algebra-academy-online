...
import { Check, Calendar, Clock, PoundSterling } from "lucide-react";
...
const PaymentSuccessPage = () => {
  ...
  useEffect(() => {
    const getBookingDetails = async () => {
      console.log("PaymentSuccess: Starting booking lookup", {
        sessionId,
        bookingId,
        user,
      });
      ...
      console.log("PaymentSuccess: Session check:", {
        session: session?.user,
        sessionError,
      });
      setDebugInfo((prev) => ({
        ...prev,
        sessionCheck: { session: session?.user, sessionError },
      }));
      ...
      if (!session?.user) {
        ...
        try {
          console.log(
            "PaymentSuccess: Attempting to fetch booking without auth context"
          );
      ...
      try {
        console.log(
          "PaymentSuccess: Fetching booking with authenticated user:",
          session.user.id
        );
        setDebugInfo((prev) => ({
          ...prev,
        ...
        if (data) {
          ...
          // Note: Do not automatically update payment status to "paid"
          // Payment will be processed automatically 24 hours before the lesson
          // This ensures the user still has the option to cancel with refund
          console.log(
            "PaymentSuccess: Booking confirmed, payment will be processed 24h before lesson"
          );
        } else {
          ...
          setDebugInfo((prev) => ({
            ...prev,
            adminQuery: "Not available - RPC function does not exist",
          }));
        }
      } catch (error) {
  ...
  const getServiceName = (serviceType: string) => {
    const services = {
      ...
      "exam-prep":
        language === "en" ? "GCSE Retakes" : "Preparação para Exames",
    };
          ...
          {loading ? (
            ...
            <Card className="shadow-lg border-green-100">
              <CardHeader className="bg-green-50">
                ...
                <CardDescription>
                  {language === "en"
                    ? "Your tutoring session information"
                    : "Informações da sua sessão de tutoria"}
                </CardDescription>
              </CardHeader>
              <CardContent className="pt-6 space-y-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="font-medium">
                    {language === "en" ? "Total Amount" : "Valor Total"}
                  </span>
                  ...
                  <div className="text-right">
                    ...
                    {booking.payment_status === "pending" && (
                      <div className="text-xs text-gray-600 mt-1">
                        {language === "en"
                          ? `Charge in: ${getTimeUntilPayment(
                              booking.lesson_date,
                              booking.lesson_time
                            )}`
                          : `Cobrança em: ${getTimeUntilPayment(
                              booking.lesson_date,
                              booking.lesson_time
                            )}`}
                      </div>
                ...
                <div className="text-sm text-green-600 font-medium mt-6">
                  <p>{getPaymentTimingMessage(language).fullMessage}</p>
                </div>
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium text-blue-900 mb-1">
                          {language === "en"
                            ...
                            : "Cronograma de Processamento do Pagamento"}
                        </h4>
                        <p className="text-sm text-blue-800">
                          {language === "en"
                            ? "Your payment method has been validated and the amount reserved. Payment will be processed 24 hours before your lesson. You can cancel up to 24 hours before the scheduled time without charge."
                            : "Seu método de pagamento foi validado e o valor foi reservado. O pagamento será processado 24 horas antes da sua aula. Você pode cancelar até 24 horas antes do horário agendado sem cobrança."}
                        </p>
                        <div className="mt-2 text-sm font-medium text-blue-900">
                          {language === "en"
                            ? `Charge scheduled for: ${getTimeUntilPayment(
                                booking.lesson_date,
                                booking.lesson_time
                              )}`
                            : `Cobrança agendada para: ${getTimeUntilPayment(
                                booking.lesson_date,
                                booking.lesson_time
                              )}`}
                        </div>
